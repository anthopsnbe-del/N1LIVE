...
(function(root){
 const initial=catalog=>({version:2,accessories:{},revision:0,credits:250,soil:12,water:6,feed:6,seeds:Object.fromEntries(catalog.filter(v=>!v.parents.length).map(v=>[v.id,2])),pots:Array.from({length:4},()=>({soil:false,plant:null})),buds:{},harvests:{},discoveries:{},total:0,history:[],seen:[]});
 function apply(state,action,p,now,catalog){
  ...
  if(s.version<2){for(const id of ['diesel','peche','pin','orchidee'])s.seeds[id]=(s.seeds[id]||0)+2;s.version=2;}s.accessories=s.accessories||{};
  switch(action){
   ...
   case 'cross':{const a=cat[p.a],b=cat[p.b];check(a&&b&&p.a!==p.b&&!a.parents.length&&!b.parents.length,'Deux fondatrices différentes sont nécessaires.');check(s.harvests[p.a]>0&&s.harvests[p.b]>0,'Récolte les deux parents d’abord.');check(s.seeds[p.a]>0&&s.seeds[p.b]>0,'Une graine de chaque parent est nécessaire.');check(s.credits>=60,'60 points nécessaires.');const id=[p.a,p.b].sort().join('--');check((s.seeds[id]||0)<9999,'Réserve pleine.');s.seeds[p.a]--;s.seeds[p.b]--;s.credits-=60;s.seeds[id]=(s.seeds[id]||0)+1;s.discoveries[id]=true;event('Croisement réussi : '+cat[id].name+'.');}break;
