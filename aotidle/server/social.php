...
require_once __DIR__.'/db.php';require_once __DIR__.'/social-core.php';require_once __DIR__.'/wallet-core.php';require_once __DIR__.'/boss-core.php';require_once __DIR__.'/war-core.php';require_once __DIR__.'/season-core.php';require_once __DIR__.'/market-core.php';require_once __DIR__.'/clan-core.php';
try{
  ...
  $pdo=db();
  /* Toutes les créations de tables se font AVANT la transaction : sur MySQL une
     instruction DDL valide implicitement la transaction en cours, et le commit
     final échouerait. */
  social_install($pdo);wallet_install($pdo);boss_install($pdo);war_install($pdo);season_install($pdo);market_install($pdo);clan_install($pdo);
  if(($in['action']??'')==='ping'){echo json_encode(['ok'=>true]+social_ping($pdo),JSON_UNESCAPED_UNICODE);exit;}
